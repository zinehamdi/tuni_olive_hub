...
class EzzitouniSocialEngine
{
    ...
    public function getInitialGreeting(string $message, ?string $postText = null): string
    {
        ...
        // 4. Scenario: Export Specifications / Cahier des charges (كراس شروط التصدير)
        if (preg_match('/(كراس|شروط|تصدير|قائمة المصدرين|رائد رسمي|وزارة الفلاحة|cahier|export)/u', $lowerMsg) || 
            preg_match('/(كراس|شروط|تصدير|قائمة المصدرين|رائد رسمي)/u', $lowerPost)) {
            return "عسلامة ومرحباً بيك في منصة زيت الزيتون التونسي 🫒! تواصلنا معاك بخصوص كراس شروط وإجراءات التصدير.. تحب نبعثلك الشروط الإدارية ولا كراس الشروط الرسمي؟";
        }

        // 5. Scenario: Organic / Bio Olive Oil (الزيت البيولوجي والعضوي)
        if (preg_match('/(بيو|بيولوجي|عضوي|bio|organic|منحة|تسميد|ecocert)/u', $lowerMsg) || 
            preg_match('/(بيو|بيولوجي|عضوي|bio|organic)/u', $lowerPost)) {
            return "عسلامة ومرحباً بيك في منصة زيت الزيتون التونسي 🫒! تواصلنا معاك بخصوص الزيت البيولوجي (العضوي).. تحب تعرف معايير الشهادة وفترة التحول ولا المنح الحكومية المخصصة للبيو؟";
        }

        // 6. Scenario: Varieties (أصناف الزيتون - شملالي، شتوي)
        if (preg_match('/(شتوي|شملالي|صنف|أصناف|نوع|وسلاتي|زلماطي|زرازي|varieties)/u', $lowerMsg) || 
            preg_match('/(شتوي|شملالي|أصناف|نوعية)/u', $lowerPost)) {
            return "عسلامة ومرحباً بيك في منصة زيت الزيتون التونسي 🫒! تواصلنا معاك بخصوص أصناف الزيتون التونسي.. تحب تعرف الفرق بين الشملالي والشتوي في نسبة الزيت ولا في الجودة والبوليفينول؟";
        }

        // 7. Scenario: Yield / Tabouiz (التبويز ونسبة استخراج الزيت)
        if (preg_match('/(تبويز|استخراج|مردود|ويبة|قنطار|rendement|extraction)/u', $lowerMsg) || 
            preg_match('/(تبويز|استخراج|مردود|عصر)/u', $lowerPost)) {
            return "عسلامة ومرحباً بيك في منصة زيت الزيتون التونسي 🫒! تواصلنا معاك بخصوص التبويز ونسبة استخراج الزيت.. شنية الصنف والولاية اللي تحب تحسب نسبة استخراجها؟";
        }

        // 8. Scenario: Shipping Contracts (عقود الشحن FOB / CIF / Flexitank)
        if (preg_match('/(fob|cif|exw|فليكسي|شحن|حاوية|كونتينار|ميناء|shipping|incoterm)/u', $lowerMsg) || 
            preg_match('/(fob|cif|شحن|فليكسي|تصدير)/u', $lowerPost)) {
            return "عسلامة ومرحباً بيك في منصة زيت الزيتون التونسي 🫒! تواصلنا معاك بخصوص عقود الشحن والتصدير.. تلوج على شحن صب فليكسي تانك ولا زيت معلب في قوارير؟";
        }

        // 9. Scenario: Prices / Market
        ...
        // 10. Scenario: Buying / Sourcing / Export
        if (preg_match('/(نشري|شراء|شاري|توزيع|استيراد|achat|buy)/u', $lowerMsg) || 
            preg_match('/(توزيع|شراء|استيراد)/u', $lowerPost)) {
            return "مرحباً بيك في منصة زيت الزيتون التونسي 🫒! قداش الكمية ونوعية الزيت (بكر ممتاز، معلب، أو صب) اللي تلوج عليها؟";
        }

        // 11. Scenario: Selling harvest / Farmer / Mill
        ...
        // 12. Default friendly single open question

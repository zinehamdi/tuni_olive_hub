    ...
    Route::get('/', function () {
        ...
        $listingsPayload = $featuredListings->map(function ($listing) {
            ...
            return [
                ...
                'unit' => ($listing->is_saniya && ($listing->unit === 'سانية' || empty($listing->unit))) ? 'ton' : ($listing->unit ?? $listing->product?->unit ?? ''),
                'packaging' => $listing->packaging,
                'sale_mode' => $listing->sale_mode ?? ($listing->is_saniya ? 'saniya' : null),
                'price_mode' => $listing->price_mode ?? ($listing->is_saniya ? 'whole' : 'per_unit'),

...
<div class="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100 py-8">
    <div class="max-w-4xl mx-auto px-4">
        <div class="flex items-center justify-between mb-8">
            <h1 class="text-3xl font-bold text-gray-900">{{ __('Create Article') }}</h1>
            <a href="{{ route('admin.articles.index') }}" class="text-gray-500 hover:text-gray-700">
                ← {{ __('Back') }}
            </a>
        </div>

        <form action="{{ route('admin.articles.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-xl p-8 space-y-6">
            @csrf
            ...
            <div class="border-b pb-6 mb-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="title[{{ $lang }}]" value="{{ old('title.'.$lang) }}" class="w-full rounded-xl border-gray-300 focus:border-[#6A8F3B] focus:ring focus:ring-[#6A8F3B]/20" required>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Category') }} ({{ $lang }})</label>
                        <input type="text" name="category[{{ $lang }}]" value="{{ old('category.'.$lang) }}" class="w-full rounded-xl border-gray-300 focus:border-[#6A8F3B] focus:ring focus:ring-[#6A8F3B]/20" required>
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('Content') }} ({{ $lang }})</label>
                        <textarea name="content[{{ $lang }}]" rows="5" class="w-full rounded-xl border-gray-300 focus:border-[#6A8F3B] focus:ring focus:ring-[#6A8F3B]/20" required>{{ old('content.'.$lang) }}</textarea>
                    </div>
                </div>
            </div>
            @endforeach
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="is_active" id="is_active" value="1" class="rounded text-[#6A8F3B] focus:ring-[#6A8F3B]" {{ old('is_active', true) ? 'checked' : '' }}>

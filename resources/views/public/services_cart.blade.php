...
<div class="min-h-screen pt-24 pb-20 px-4 bg-gray-50">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-12 text-center border border-gray-100">
            ...
            <p class="text-gray-500 mb-6">{{ __('You have not added any services to your cart yet.') }}</p>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-6 md:p-8">
                <div class="space-y-6">
                    ...
                    <div class="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 pb-6 border-b border-gray-100 last:border-0 last:pb-0">
                        ...
                        <div class="flex-grow text-center sm:text-{{ __('left') }}">
                            <h3 class="text-lg font-bold text-gray-900">{{ $item['name'] }}</h3>
                            <p class="text-sm text-gray-500 mt-1">{{ __('Quantity') }}: {{ $item['quantity'] }}</p>
                        </div>
            ...
            <div class="bg-gray-50 p-6 md:p-8 border-t border-gray-100">
                <form action="{{ route('services.checkout') }}" method="POST" onsubmit="trackCheckoutInitiate({{ $total }})">
                    @csrf
                    <div class="mb-6 bg-blue-50 border-l-4 border-blue-400 p-4 rounded-r text-blue-800 text-sm leading-relaxed">
                        {{ __('We need some information about your business to build your campaign. The campaign budget is determined by your selected packages. Please fill out the form below to book an appointment to discuss the details.') }}
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('Name') }} <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('Phone Number') }} <span class="text-red-500">*</span></label>
                    ...
                    <div class="mb-5">
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('Business Information') }} <span class="text-red-500">*</span></label>
                        <textarea name="business_info" required rows="3" placeholder="{{ __('Facebook page, website, product descriptions...') }}" class="w-full rounded-xl border-gray-300 focus:border-[#6A8F3B] focus:ring focus:ring-[#6A8F3B]/20"></textarea>
                        @error('business_info') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                    </div>

                    <div class="mb-8">
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">{{ __('Preferred Appointment Date & Time') }} <span class="text-red-500">*</span></label>
                        <input type="datetime-local" name="appointment_date" required min="{{ date('Y-m-d\TH:i', strtotime('+1 day')) }}" class="w-full rounded-xl border-gray-300 focus:border-[#6A8F3B] focus:ring focus:ring-[#6A8F3B]/20">
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-center gap-4 border-t border-gray-200 pt-6">
                        <div>
                            <p class="text-sm text-gray-500 mb-1">{{ __('Total Campaign Budget') }}</p>

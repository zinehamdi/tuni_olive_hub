...
class ChatbotController extends Controller
{
    ...
    public function chat(Request $request)
    {
        ...
        // Prepare conversation history and limit to last 6 to save quota
        $history = $request->input('history', []);
        if (is_array($history) && count($history) > 6) {
        ...
        // 1. Bypass Gemini API entirely if we are currently inside a multi-step fallback flow
        if (session()->has('chatbot_state')) {
            return response()->json([
                'reply' => $this->handleFallbackIntent($request->input('message'))
            ]);
        }

        // 2. Intercept specific intents to force them into our robust state machine
        $userMessageStr = strtolower(trim($request->input('message')));
        if (preg_match('/(تسجيل|حساب|انضمام|اشتراك|نعمل كونط|نحب نقيد|نسجل|قيدني|nsajel|compte|n7eb n9ayed|n9ayed|na3mel compte|inscrire|inscription|register|signup|join|بيع|إضافة منتج|زيتون|زيت|نحب نبيع|نهبط سلعة|نصب زيت|nbi3|nhebb nbi3|nhabbat sel3a|zite|zitoun|bi3|vendre|ajouter|produit|sell|add product|olive oil|تصدير|ديوانة|لبرة|نصدر|tasdir|nsader|lbarra|export|diwana|exporter|exportation|customs|كراس الشروط|كيفاش|الشروط|القوانين|كيفاه|korraset chourout|koraset|chourout|kifech|kiféh|kifeh|cahier des charges|comment|conditions|how|requirements|اتصال|تواصل|مساعدة|نكلمكم|عاونوني|مساعدة|مشكلة|nkalmkom|mouchkla|help|contact|aide|problème|appeler)/ui', $userMessageStr)) {
            return response()->json([
                'reply' => $this->handleFallbackIntent($request->input('message'))
            ]);
        }
        ...
        try {
            ...
            // Trigger Fallback on Quota or High Demand
            if ($status === 429 || $status === 503 || str_contains(strtolower($geminiError), 'quota') || str_contains(strtolower($geminiError), 'limit') || str_contains(strtolower($geminiError), 'demand')) {
                $reply = $this->handleFallbackIntent($request->input('message'));
    ...
    /**
     * Intelligent Fallback matching for when the AI is down or quota is exhausted.
     * Uses session state to handle multi-step flows like Registration.
     */
    private function handleFallbackIntent($message)
    {
        ...
        if (preg_match('/(تسجيل|حساب|انضمام|اشتراك|نعمل كونط|نحب نقيد|نسجل|قيدني|nsajel|compte|n7eb n9ayed|n9ayed|na3mel compte|inscrire|inscription|register|signup|join)/ui', $message)) {
            ...
            return 'يسعدنا انضمامك لمنصة ZinToop!<br><br><b>أولاً، ما هو دورك في المنصة؟ (اضغط على خيار أو اكتبه)</b><br>' .
                   ...
                   '<button type="button" onclick="const i = document.querySelector(\'div[x-data=\\\'ezzitouniChat()\\\'] input[type=\\\'text\\\']\'); i.value=\'معبئ\'; i.dispatchEvent(new Event(\'input\', { bubbles: true })); document.querySelector(\'div[x-data=\\\'ezzitouniChat()\\\'] button[type=\\\'submit\\\']\').click();" class="bg-gray-100 border border-gray-200 text-gray-800 p-2 rounded-xl text-sm hover:bg-gray-200 transition text-right">📦 معبئ</button>' .
